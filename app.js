...
input.addEventListener("keyup", function(event) {
  if (event.keyCode === 13) {
    ...
    var parent = document.getElementById("language");
...
function search() {
  inputQuery = document.getElementById("searchInput").value+"["+language+"]";
  if (true) {
    oReq = new XMLHttpRequest();
    oReq.addEventListener("load", reqListener);
    oReq.open(
      "POST",
      `https://apis.paralleldots.com/v3/keywords?api_key=YOUR-API-KEY&text=${inputQuery}`
    );
    oReq.send();
  } else {
    ...
    utubeReq.open(
      "GET",
      `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${utubeQuery}&maxResults=10&type=video&videoCaption=closedCaption&key=YOUR-API-KEY`
    );
